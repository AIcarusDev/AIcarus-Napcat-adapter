...
class RecallMessageHandler(BaseActionHandler):
    ...
    async def execute(self, action_seg: Seg, event: Event, send_handler: "SendHandlerAicarus") -> Tuple[bool, str, Dict[str, Any]]:
        ...
        try:
            response = await send_handler._send_to_napcat_api("delete_msg", {"message_id": int(target_message_id)})
            if response and response.get("status") == "ok":
                return True, "消息已成功传达撤回指令", {}
            else:
                error_msg = response.get("message", "Napcat API 错误") if response else "无响应"
...
class PokeUserHandler(BaseActionHandler):
    ...
    async def execute(self, action_seg: Seg, event: Event, send_handler: "SendHandlerAicarus") -> Tuple[bool, str, Dict[str, Any]]:
        ...
        if not target_gid_str and event.conversation_info and event.conversation_info.type == ConversationType.GROUP:
        ...
        try:
            ...
            response = await send_handler._send_to_napcat_api(napcat_action_name, params_poke)
            if response and response.get("status") == "ok":
                return True, "戳一戳指令已发送", {}
            else:
                error_msg = response.get("message", f"Napcat API for {napcat_action_name} error") if response else "无响应"
                return False, error_msg, {}
        except ValueError:
            return False, f"无效的 user_id 或 group_id 格式: {target_uid_str}, {target_gid_str}", {}
        except Exception as e:
...
class HandleFriendRequestHandler(BaseActionHandler):
    ...
    async def execute(self, action_seg: Seg, event: Event, send_handler: "SendHandlerAicarus") -> Tuple[bool, str, Dict[str, Any]]:
        ...
        try:
            params_fh: Dict[str, Any] = {"flag": request_flag, "approve": approve_action}
            if approve_action and remark:
                params_fh["remark"] = remark
            
            response = await send_handler._send_to_napcat_api("set_friend_add_request", params_fh)
            if response and response.get("status") == "ok":
                return True, "好友请求已处理", {}
            else:
                error_msg = response.get("message", "Napcat API for set_friend_add_request error") if response else "无响应"
                return False, error_msg, {}
        except Exception as e:
...
class HandleGroupRequestHandler(BaseActionHandler):
    ...
    async def execute(self, action_seg: Seg, event: Event, send_handler: "SendHandlerAicarus") -> Tuple[bool, str, Dict[str, Any]]:
        ...
        try:
            ...
            response = await send_handler._send_to_napcat_api("set_group_add_request", params_gh)
            if response and response.get("status") == "ok":
                return True, "群请求已处理", {}
            else:
                error_msg = response.get("message", "Napcat API for set_group_add_request error") if response else "无响应"
                return False, error_msg, {}
        except Exception as e:
