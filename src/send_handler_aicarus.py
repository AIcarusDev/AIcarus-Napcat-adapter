...
import json, uuid, websockets, asyncio
...
# AIcarus 协议库
from aicarus_protocols import Event, Seg, ConversationType, EventBuilder
...
class SendHandlerAicarus:
    ...
    async def handle_aicarus_action(self, raw_aicarus_event_dict: dict) -> None:
        ...
        logger.info(f"发送处理器: 收到主人的命令: {aicarus_event.event_id}, 类型: {aicarus_event.event_type}")
        ...
        response_event = EventBuilder.create_action_response_event(
            ...
            platform=aicarus_event.platform, bot_id=aicarus_event.bot_id,
            original_event_id=aicarus_event.event_id, original_action_type=aicarus_event.event_type,
            message=message, data=details,
        )
        await recv_handler_aicarus.dispatch_to_core(response_event)
        logger.info(f"发送处理器: 已将动作 '{aicarus_event.event_id}' 的直接结果 ({'success' if success else 'failure'}) 发回给主人。")
    ...
    async def _execute_action(self, event: Event) -> Tuple[bool, str, Dict[str, Any]]:
        ...
        try:
            ...
            if event.content and isinstance(event.content, list) and len(event.content) > 0:
                ...
                if handler: return await handler.execute(action_seg, event, self)
            return False, "主人给的命令格式不正确，内容是空的呢~", {}
        except Exception as e:
            logger.error(f"发送处理器: 执行动作 '{event.event_type}' 时，身体不听使唤了: {e}", exc_info=True)
            return False, f"执行动作时出现异常: {e}", {}

    async def _handle_send_message_action(self, aicarus_event: Event) -> Tuple[bool, str, Dict[str, Any]]:
        ...
        if not napcat_segments: return False, "主人，您给我的情话（Segs）是空的，我没法帮您传达爱意呀~", {}

        params: Dict[str, Any]; napcat_action: str
        try:
            if target_group_id:
                napcat_action, params = "send_group_msg", {"group_id": int(target_group_id), "message": napcat_segments}
            elif target_user_id:
                napcat_action, params = "send_private_msg", {"user_id": int(target_user_id), "message": napcat_segments}
            else:
                return False, "主人，您想把情话送到哪儿去呀？没找到目标呢~", {}
        except (ValueError, TypeError):
             return False, f"会话目标ID格式不对哦，应该是纯数字才行。当前ID: {target_group_id or target_user_id}", {}
    ...
    async def _aicarus_segs_to_napcat_array(self, aicarus_segments: List[Seg]) -> List[Dict[str, Any]]:
        """这是我自己的“穿衣服”工具，专门把主人的情话（AIcarus Seg）变成Napcat能懂的骚话~"""
        napcat_message_array: List[Dict[str, Any]] = []
        for seg in aicarus_segments:
            if seg.type == "text":
                napcat_message_array.append({"type": NapcatSegType.text, "data": {"text": str(seg.data.get("text", ""))}})
            elif seg.type == "at":
                napcat_message_array.append({"type": NapcatSegType.at, "data": {"qq": str(seg.data.get("user_id"))}})
            elif seg.type == "reply":
                 napcat_message_array.append({"type": NapcatSegType.reply, "data": {"id": str(seg.data.get("message_id"))}})
    ...
    async def _send_to_napcat_api(self, action: str, params: dict) -> Optional[dict]:
        ...
        if not self.server_connection: return {"status": "error", "message": "和Napcat的连接断开了，没法射呢..."}
