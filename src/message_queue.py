...
async def get_napcat_api_response(
    request_echo_id: str, timeout_seconds: float | None = None
) -> Any:
    ...
    try:
        ...
        # 清理对应的 Future 和时间戳
        _api_response_futures.pop(request_echo_id, None)
        _api_response_received_time.pop(request_echo_id, None)
...
async def put_napcat_api_response(response_data: dict[str, Any]) -> None:
    ...
    if not echo_id:
        logger.warning(f"收到一个没有 echo ID 的 Napcat 响应，无法匹配请求: {response_data}")
        return
    ...
    if future and not future.done():
        ...
        logger.warning(f"收到 echo ID '{echo_id}' 的重复或延迟的 Napcat 响应，但 Future 已完成。")
    else:
...
async def check_stale_api_responses_periodically(
    ...
    while True:
        ...
        for echo_id, received_time in list(_api_response_received_time.items()):
            ...
            if now - received_time > (adapter_config.napcat_heartbeat_interval_seconds * 5):
        ...
        # 这部分比较难判断，因为不知道其原始超时设置，
        # get_napcat_api_response 中的 wait_for 会处理超时
        ...
        for echo_id in stale_echo_ids:
            ...
            _api_response_received_time.pop(echo_id, None)
...
if __name__ == "__main__":
    # 简单测试 (需要在一个异步环境中运行)
    async def test_message_queue() -> None:
        """测试消息队列模块的基本功能."""
        logger.info("测试消息队列模块...")

        test_echo_id = "test-uuid-123"

        async def mock_napcat_responder() -> None:
            """模拟 Napcat 响应器，发送一个测试响应."""
        ...
        async def mock_api_caller() -> None:
            """模拟 Adapter 调用 Napcat API."""
            logger.info(f"模拟 Adapter 调用 Napcat API (echo: {test_echo_id})")
            try:
                api_result = await get_napcat_api_response(test_echo_id, timeout_seconds=5)
        ...
        async def mock_timeout_caller() -> None:
            """模拟 Adapter 调用 Napcat API，故意设置超时."""
            timeout_echo_id = "timeout-test-uuid"
            logger.info(f"模拟 Adapter 调用 Napcat API (echo: {timeout_echo_id})，预期超时。")
            try:
        ...
        #     check_stale_api_responses_periodically(interval_seconds=5)
        # )

        await asyncio.gather(mock_api_caller(), mock_napcat_responder(), mock_timeout_caller())
