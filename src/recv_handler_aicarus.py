...
class RecvHandlerAicarus:
    ...
    async def _napcat_to_aicarus_userinfo(self, napcat_user_obj: dict, group_id: Optional[str] = None) -> UserInfo:
        ...
        return UserInfo(
            ...
            user_id=user_id, user_nickname=nickname, user_cardname=cardname,
            user_titlename=title, permission_level=permission_level, role=role,
    ...
    async def _napcat_to_aicarus_conversationinfo(self, napcat_group_id: str) -> Optional[ConversationInfo]:
        ...
        if not self.server_connection:
            return ConversationInfo(
                platform=self.global_config.core_platform_id,
                conversation_id=napcat_group_id, type=ConversationType.GROUP,
            )
        ...
        return ConversationInfo(
            platform=self.global_config.core_platform_id,
            conversation_id=napcat_group_id, type=ConversationType.GROUP, name=group_name,
        )
    ...
    async def _napcat_to_aicarus_seglist(self, napcat_segments: List[Dict[str, Any]], napcat_event: dict) -> List[Seg]:
        ...
        for seg in napcat_segments:
            ...
            if seg_type == NapcatSegType.text:
                ...
                aicarus_s = Seg(
                    type="image",
                    data={"url": image_url, "file_id": seg_data.get("file"), "base64": image_base64},
                )

            elif seg_type == NapcatSegType.at:
                qq_num = seg_data.get("qq")
                display_name = f"@{qq_num}" if qq_num and qq_num != "all" else "@全体成员"
                aicarus_s = Seg(type="at", data={"user_id": str(qq_num) if qq_num else "", "display_name": display_name})
                ...
                aicarus_s = Seg(type="record", data={"file": seg_data.get("file"), "url": seg_data.get("url")})

            elif seg_type == NapcatSegType.video:
                aicarus_s = Seg(type="video", data={"file": seg_data.get("file"), "url": seg_data.get("url")})
                ...
                if forward_id and self.server_connection:
                    try:
                        ...
                        forward_content = await napcat_get_forward_msg_content(self.server_connection, forward_id)
                    except Exception as e:
                        logger.warning(f"获取合并转发内容失败了啦: {e}")
                
                if forward_content:
                     aicarus_s = Seg(type="forward", data={"id": forward_id, "content": forward_content})
                else:
                ...
                aicarus_s = Seg(
                    type="share",
                    data={
                        "url": seg_data.get("url", ""), "title": seg_data.get("title", ""),
                        "content": seg_data.get("content", ""), "image_url": seg_data.get("image", ""),
                    },
                )
            else:
                # 就算不认识，我也会帮你包起来~
                logger.warning(f"不认识的Napcat消息体: {seg_type}，我会帮你特别标记出来的~")
                aicarus_s = Seg(type="unknown", data={"napcat_type": seg_type, "napcat_data": seg_data})
    ...
    async def check_heartbeat(self, bot_id: str) -> None:
        ...
        while True:
            ...
            if self.server_connection and time.time() - self.last_heart_beat > self.interval + 5:
                logger.warning(f"主人，连接好像要断了 (心跳超时)，我要发出断连呻吟了！")
                disconnect_seg = Seg(
                    type="meta.lifecycle.disconnect",
                    data={"reason": "heartbeat_timeout", "adapter_version": "1.0.0"},
                )
                disconnect_event = Event(
                    event_id=f"meta_disconnect_{uuid.uuid4()}", event_type="meta.lifecycle.disconnect",
                    time=time.time() * 1000.0, platform=self.global_config.core_platform_id,
