...
class CoreConnectionClient:
    ...
    async def _connect(self) -> bool:
        ...
        try:
            ...
            logger.info(f"准备向 Core 发送 meta.lifecycle.connect 事件 (同时用于注册)，Adapter ID: '{adapter_id_for_registration}'")
            ...
            logger.info(f"已向 Core 发送 meta.lifecycle.connect 事件 (Adapter ID: {adapter_id_for_registration})，此事件将用于注册。")
            return True
        except InvalidURI:
            ...
            logger.error(f"连接 Core ({self.core_ws_url}) 时发生 WebSocket 异常: {e}", exc_info=True)
    ...
    async def _heartbeat_loop(self) -> None:
        """定期向 Core 发送心跳包。"""
        logger.info(f"心跳循环准备启动 (Adapter ID: {self.platform_id})，每 {self.heartbeat_interval} 秒发送一次。")
        try:
            while self._is_running and self.websocket and self.websocket.open:
                await asyncio.sleep(self.heartbeat_interval)
                if not self._is_running: logger.debug("心跳循环：_is_running 为 false，退出。"); break
                if not self.websocket: logger.debug("心跳循环：websocket 为 None，退出。"); break
                if not self.websocket.open: logger.debug("心跳循环：websocket 连接已关闭，退出。"); break

                heartbeat_payload = {"type": "heartbeat", "adapter_id": self.platform_id}
                try:
                    ...
                    logger.warning(f"发送心跳包到 Core 失败: {e}. 连接可能已断开，心跳循环将终止。")
                    break 
            ...
            logger.error(f"心跳循环意外终止 (Adapter ID: {self.platform_id}): {e_outer}", exc_info=True)
        finally:
        ...
        try:
            while self._is_running and self.websocket and self.websocket.open:
                try:
                    ...
                    logger.error(f"接收来自 Core 的消息时发生 WebSocket 异常: {e_ws_recv}", exc_info=True)
                    break
            ...
            logger.info(f"消息接收循环被取消 (Adapter ID: {self.platform_id}).")
             ...
             logger.error(f"消息接收循环意外终止 (Adapter ID: {self.platform_id}): {e_outer_recv}", exc_info=True)
        finally:
    ...
    async def run_forever(self) -> None:
        ...
        while self._is_running:
            if await self._connect():
                self._receive_task = asyncio.create_task(self._receive_loop(), name=f"ReceiveTask-{self.platform_id}")
                self._heartbeat_task = asyncio.create_task(self._heartbeat_loop(), name=f"HeartbeatTask-{self.platform_id}")
                    ...
                    if not task.done():
                        ...
                        try:
                            ...
                            logger.debug(f"任务 {task.get_name()} 在等待完成时被成功取消。")
                             ...
                             logger.error(f"等待挂起任务 {task.get_name()} 完成时发生错误: {e_pending_await}", exc_info=True)
                ...
                for task in done:
                    try:
                        ...
                        logger.error(f"任务 {task.get_name()} 异常结束: {e_task_done}", exc_info=True)
                ...
                if self.websocket:
                    try:
                        ...
                        pass 
                    self.websocket = None
                logger.info(f"与 Core 的连接已断开或相关任务已停止 for Adapter ID: {self.platform_id}.")

            if self._is_running:
                logger.info(f"与 Core 的连接已断开，将在 {self._reconnect_delay} 秒后尝试重连 (Adapter ID: {self.platform_id})...")
                await asyncio.sleep(self._reconnect_delay)
            else:
                logger.info(f"Core 通信层被外部信号停止，不再重连 (Adapter ID: {self.platform_id}).")
                break
    ...
    async def stop_communication(self) -> None:
        ...
        for task in tasks_to_cancel:
            ...
            try:
                ...
                logger.error(f"等待任务 {task.get_name()} 取消时发生错误: {e_cancel_task}", exc_info=True)
        ...
        if self.websocket and self.websocket.open:
            try:
                # 发送主动断开连接的事件给Core
                logger.info(f"Adapter ({self.platform_id}) 准备主动断开连接，将发送 meta.lifecycle.disconnect 事件。")
                ...
                logger.info(f"已向 Core 发送 meta.lifecycle.disconnect 事件 (adapter_id: {self.platform_id}).")
                ...
                logger.error(f"关闭与 Core 的 WebSocket 连接或发送断开事件时发生错误: {e_close}", exc_info=True)
        self.websocket = None
            ...
            if event_type.startswith("message."):
                ...
                for seg in content:
                    if isinstance(seg, dict):
                        ...
                        if seg_type == "text": text = seg.get("data", {}).get("text", ""); simplified_content.append(text[:50] + "..." if len(text) > 50 else text)
                        elif seg_type == "image": simplified_content.append("[图片]")
                        elif seg_type == "face": face_name = seg.get("data", {}).get("name", "[表情]"); simplified_content.append(face_name)
                        elif seg_type == "at": user_id = seg.get("data", {}).get("user_id", ""); simplified_content.append(f"@{user_id}")
                        elif seg_type == "record": simplified_content.append("[语音]")
                        elif seg_type == "video": simplified_content.append("[视频]")
                        elif seg_type == "forward": simplified_content.append("[合并转发]")
                        elif seg_type == "json_card": simplified_content.append("[JSON卡片]")
                        elif seg_type == "xml_card": simplified_content.append("[XML卡片]")
                        elif seg_type == "share": simplified_content.append("[分享]")
                        elif seg_type == "message_metadata": continue
                        else: simplified_content.append(f"[{seg_type}]")
                content_str = "".join(simplified_content)
                user_info = event_dict.get("user_info", {}); conversation_info = event_dict.get("conversation_info", {})
                ...
                if conversation_info.get("type") == "group": return f"群消息 {group_name}({user_name}): {content_str}"
                else: return f"私聊消息 {user_name}: {content_str}"
            elif event_type.startswith("notice."): return f"通知事件: {event_type}"
            elif event_type.startswith("request."): return f"请求事件: {event_type}"
            elif event_type.startswith("meta."): return f"元事件: {event_type}"
            else: return f"事件: {event_type} (ID: {event_id})"
        except Exception as e: return f"事件解析错误: {e}"
    ...
    async def send_event_to_core(self, event_dict: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            logger.error(f"序列化发送给 Core 的事件时出错: {e_json}. 事件内容: {event_dict}", exc_info=True)
            return False
    ...
    async def main_test():
        ...
        if core_connection_client.websocket and core_connection_client.websocket.open:
            ...
            test_event_to_core = Event(
                event_id=f"test_msg_{uuid.uuid4()}", event_type="message.private.friend",
                time=int(time.time() * 1000), platform=get_config().core_platform_id,
                bot_id="test_bot_from_adapter", user_info=None, conversation_info=None,
                content=[SegBuilder.text("你好，Core！来自 Adapter 的测试消息 (v1.4.0)。")],
        ...
        if comm_task and not comm_task.done():
            comm_task.cancel()
            try: await comm_task
            except asyncio.CancelledError: logger.info("通信任务已取消。")
